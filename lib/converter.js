 ...
 * @returns {ReturnType<typeof ffmpeg>}
 */
function toImg(buffer, ext) {
    return ffmpeg(buffer, [
        '-vf', 'scale=512:512:flags=lanczos:force_original_aspect_ratio=decrease,format=rgba,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=white'
    ], ext, 'png')
}
...
function toGif(buffer, ext) {
    return ffmpeg(buffer, [
        '-vf', 'scale=512:512:flags=lanczos:force_original_aspect_ratio=decrease,fps=15,format=rgb24,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=white'
    ], ext, 'gif')
}
