            ...
            if (stat.isDirectory()) {
                ...
                try {
                    ...
                    if (plugin.default && typeof plugin.default === 'object') {
                        ...
                        // Colored plugin loading log
                        const categoryColor = category ? getCategoryColor(category) : chalk.white
                        const pluginName = category ? `${category}/${item}` : item
                        console.log(chalk.green('  ✅') + chalk.gray(' Loaded: ') + categoryColor(pluginName))                    }
                } catch (error) {
                    console.error(chalk.red('  ❌') + chalk.gray(' Failed: ') + chalk.white(`${category ? `${category}/` : ''}${item}`) + chalk.red(` - ${error.message}`))
                }
            }
        }
    }
