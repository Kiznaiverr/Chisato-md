...
export class Handler {
    constructor(sock, db, plugins) {
        ...
        try {
            ...
            const pushName = msg.pushName || 'Unknown'            // Create message context
            const context = {
                sock: this.sock,
                msg,
                body,
            ...
            const botPrefix = config.getPrefix()
        ...
        if (plugin.limit && !this.db.isOwner(sender) && !this.db.isPremium(sender)) {
            if (!this.db.useLimit(sender, plugin.limit)) {
                const user = this.db.getUser(sender)
                const logMsg = `${timeStr} 🔴 LIMIT ⏱️ ${userName} → ${prefix}${command} (No limit remaining: ${user.limit})`
                console.log(logMsg)
